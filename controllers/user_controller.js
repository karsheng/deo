...
module.exports = {
	getRegistrationInfo(req, res, next) {
		...
		Registration.findOne({ _id: registration_id, user })
	...
	registerForEvent(req, res, next) {
		...
		checkRegistrationEligibity(participant, category, next, function(
			errMessage,
			isEligible
		) {
			if (isEligible) {
						...
						if (unpaidReg) {
							...
							Participant.findByIdAndUpdate(unpaidReg.participant, participant)
								.then(updatedParticipant => {
									unpaidReg.save().then(reg => res.json(reg)).catch(next);
								})
								.catch(next);
						} else {
							...
							Promise.all([p.save(), registration.save()])
		...
		.then(category => {
			Registration.checkStatus(category, function(err, isOpen) {
				...
				if (isOpen) {
					...
					return cb(
						{ message: 'Registration for this category is closed' },
						false
					);
				}
			});
